...
const MyCart: FunctionComponent = () => {
  ...
  const grouped: Record<string, any[]> = items.reduce((acc, it) => {
            ...
            {Object.keys(grouped).map((rid) => (
              <div key={rid} className="bg-white rounded-2xl shadow p-4">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="font-semibold">{grouped[rid][0]?.restaurantName || "Restaurant"}</div>
                  <div className="ml-auto text-sm text-muted-foreground"> </div>
                </div>

                {grouped[rid].map((it) => (
                  <div key={String(it.id)} className="flex items-center justify-between py-3 border-b last:border-b-0">
                    <div className="flex items-center gap-4">
                      <img src={it.image || "/placeholder.svg"} alt={it.name} className="w-16 h-16 rounded-xl object-cover" />
                      <div>
                        <div className="font-medium">{it.name}</div>
                        <div className="font-extrabold">{formatCurrency(it.price)}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <button
                        onClick={() => dispatch(updateQty({ id: it.id, qty: Math.max(1, it.qty - 1) }))}
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div>
                    ...
                    <div className="font-extrabold text-lg">{formatCurrency(totals(grouped[rid]))}</div>
                  </div>
                  <button onClick={() => nav('/checkout')} className="rounded-full bg-red-600 text-white px-6 py-2">Checkout</button>
                </div>
              </div>
            ))}
          </div>
